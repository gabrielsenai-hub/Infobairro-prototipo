...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .csrf(csrf -> csrf.disable()) // desabilita CSRF se você usa API REST
                .cors(cors -> cors.configurationSource(corsConfigurationSource())) // usa a config abaixo
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/", "/auth/**", "/oauth2/**").permitAll()
                        .anyRequest().authenticated()           // o resto precisa de auth
                )
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOrigins(List.of("http://localhost:5500/", "http://127.0.0.1:5050")); // origem do seu front
